...
class DynDnsController extends Controller
{
    ...
    public function destroy(int $id): JsonResponse
    {
        ...
        // Check if this config is the current hostname before deleting
        $isHostname = $this->isConfigHostname($config);
        ...
        // If this was the hostname, reset to localhost and remove certificate
        if ($isHostname) {
            $this->resetHostname();

            // Remove the Let's Encrypt certificate
            $this->sslService->removeCertificate($config->full_domain);
        }
    ...
    /**
     * Set a DynDNS config's full_domain as the system hostname.
     */
    public function setHostname(int $id): JsonResponse
    {
        $config = DynDnsConfig::findOrFail($id);
        ...
        return response()->json([
            'message' => 'Hostname set to '.$config->full_domain,
            'hostname' => $config->full_domain,
        ]);
    }
    ...
    /**
     * Check if a DynDNS config matches the current system hostname.
     */
    protected function isConfigHostname(DynDnsConfig $config): bool
    {
        $hostname = $this->sslService->getCurrentHostname();

        return $hostname !== null && $hostname === $config->full_domain;
    }

    /**
     * Reset the system hostname to localhost.
     */
    ...
    protected function formatConfig(DynDnsConfig $config): array
    {
        ...
        return [
            ...
            'is_hostname' => $this->isConfigHostname($config),
