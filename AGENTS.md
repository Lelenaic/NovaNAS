...
- Every new setting MUST have a migration that inserts its initial value into the database. Never provide a default value in application code (e.g. `settings->get('key', $default)` or `$array['key'] ?? default`) — always trust the value stored in the database 100%. Migrations should insert the default only when the key does not already exist, so existing user-defined values are never overwritten.
...
### Browser testing
As you're a remote agent on a headless system, you cannot use visit() or screenshots.
